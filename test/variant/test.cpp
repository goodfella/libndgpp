...
TEST(copy_ctor, is_called)
{
    ...
    const auto visitor = [&called] (const copy_tracker& tracker) {
        return tracker.called;
    };

    EXPECT_TRUE(ndgpp::visit(visitor, copy));
}
...
TEST(move_ctor, is_called)
{
    ...
    const auto visitor = ndgpp::make_visitor([] (const bool &) {
            FAIL();
        },
...
TEST(move_ctor, other_with_value)
{
    ...
    const auto visitor = ndgpp::make_visitor([&correct_value] (const bool v) {
...
{
    {
        ...
        bool correct_value = false;
        const auto visitor = ndgpp::make_visitor([] (const int&) {
                FAIL();
            },
...
{
    {
        ...
        bool correct_value = false;
        const auto visitor = ndgpp::make_visitor([&correct_value] (const int& v) {
                correct_value = (v == 2);
            },
            [] (const double&) {
                FAIL();
...
TEST(move_assign, this_valueless_by_exception)
{
    ...
    const auto visitor = ndgpp::make_visitor(
        [&correct_value] (const double v) {
            correct_value = (v == 3.0);
        },
        [] (const int) {
...
TEST(copy_assign, different_types)
{
    ...
    const auto visitor = ndgpp::make_visitor([] (const bool v) {
            FAIL();
        },
        [&] (const double v) {
            proper_value = (v == 1.2);
